...
import com.dita.service.DiseaseService;
...
public class DiseaseExcelLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        InputStream is = getClass().getClassLoader().getResourceAsStream("templates/hospital/질병분류코드.xls");
        if (is == null) {
        ...
        try (Workbook workbook = new HSSFWorkbook(is)) {
            ...
            for (Row row : sheet) {
                if (rowCount++ == 0) continue; // 첫 줄(헤더) 스킵

                Cell codeCell = row.getCell(0); // 질병코드
                Cell nameCell = row.getCell(1); // 질병명
                ...
                diseaseService.saveIfNotExists(disease);
                System.out.println("✔ 저장 완료: " + code + " / " + name);
            }
        }
    }
}
